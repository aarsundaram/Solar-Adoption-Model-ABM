...
class AdoptionModel(Model):
    ...
    def __init__(self,filename):
        ...
        self.bgid_income_dict= {}                          ## circle-2 interaction
        self.incomegroups_dict= {}                    ## to hold list of all households in albany
    ...
    def generate_model(self):
        ...
        incomegroups = list(households_main['income'].unique())
        ...
        for geoid in list(df['GEOID10'].unique()):
            for income in incomegroups:
                self.geoid_income_dict[(geoid,income)] =[]


        for blockgroup in list(df['bgid'].unique()):
            for income in incomegroups:
                self.bgid_income_dict[(blockgroup,income)]=[]
        ...
        for agent in self.schedule.agents:
            ...
            bgid_neighs = random.choices(self.bgid_income_dict[(agent.bgid,agent.income)],k=min(50,len(self.bgid_income_dict[(agent.bgid,agent.income)])))
            ...
            thirdcircle = random.choices(self.incomegroups_dict[(agent.income)],k=200)
    ...
    def subnorms_evolution(self):
        ...
        for household in self.schedule.agents:
            ...
            adopters_in_block = []
            for hh in self.geoid_income_dict[(household.geoid,household.income)]:
            ...
            if household.adoption_status==0:
                if len(adopters_in_block) > (0.33 * len(self.geoid_income_dict[(household.geoid,household.income)])): 
                ...
                if len(adopters_in_block) > (0.5 * len(self.geoid_income_dict[(household.geoid,household.income)])): 
...
def model_run(filename):
    ...
    sample.datacollector_df.to_csv(rootpath+'experiment/segregated/scenario3b_2pp/'+str(outputfile))
    seeded_df['seeded_agents']=sample.seeded_agents
    seeded_df.to_csv(rootpath+'experiment/segregated/scenario3b_2pp/seeds/'+str(outputfile))    
